...
export default function ProductForm({ product }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      image: product?.image_url || product?.image?.[0] || "",
  ...
  async function onSubmit(data: ProductFormData): Promise<void> {
    try {
      ...
      const productData = {
        ...
        warranty_months: data.warranty_months ? Number(data.warranty_months) : 0,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
      {/* Nombre del Producto */}
      <div>
        <label className="block mb-1 text-sm font-semibold text-gray-700">Nombre del Producto *</label>
        <input
          {...register("name")}
      ...
      {/* Descripción */}
      <div>
        <label className="block mb-1 text-sm font-semibold text-gray-700">Descripción</label>
        <textarea
      ...
      {/* Clasificación: Marca, Categoría, Subcategoría y Condición */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Marca</label>
          <input
            {...register("brand")}
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Categoría</label>
          <input
            {...register("category")}
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Subcategoría</label>
          <input
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Condición</label>
          <select
            {...register("condition")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Costo ($)</label>
          <input
            type="number"
            step="0.01"
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Margen Ganancia (%)</label>
          <input
            type="number"
            step="0.01"
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Precio de Venta ($) *</label>
          <input
            type="number"
            step="0.01"
      ...
      {/* Inventario, Descuento, Peso y Garantía */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Stock (Inventario) *</label>
          <input
            type="number"
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Descuento (%)</label>
          <input
            type="number"
            step="0.01"
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Peso (Kg)</label>
          <input
            type="number"
            step="0.01"
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Garantía (Meses)</label>
          <input
            type="number"
      ...
      {/* Identificación: SKU y Slug */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">SKU (Código único)</label>
          <input
            {...register("sku")}
        ...
        <div>
          <label className="block mb-1 text-sm font-semibold text-gray-700">Slug URL (Opcional)</label>
          <input
            {...register("slug")}
            placeholder="ej-laptop-dell-xps-13"
            className="border border-gray-300 p-3 rounded-lg w-full focus:ring-2 focus:ring-cyan-500 focus:outline-none transition"
      ...
      <div className="space-y-3 pt-2">
        ...
        <ImageUploader
          imageUrl={product?.image_url || product?.image?.[0]}
          onImageChange={handleFileChange}
        />
        <div>
          <label className="block mb-1 text-xs font-medium text-gray-500">O ingresa URL de imagen directamente</label>
          <input
            {...register("image")}
      ...
      <div className="flex flex-wrap gap-6 pt-2 border-t border-gray-100">
        ...
        <label className="flex gap-2 items-center cursor-pointer select-none">
          <input 
            ...
            className="w-4 h-4 text-cyan-600 rounded focus:ring-cyan-500"
          />
          <span className="text-sm font-medium text-gray-700">Producto activo en tienda</span>
        </label>
      </div>
